...
#
#   docker build -t ipie-service-template .
#
# It was a Gradle module until then, which is why this file used to require the monorepo root as
# context and a module-qualified :ipie-service-template:bootJar task.
...
COPY --from=build /workspace/build/libs/ipie-service-template.jar app.jar
