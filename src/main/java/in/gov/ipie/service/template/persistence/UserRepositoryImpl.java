...
class UserRepositoryImpl implements UserRepository {
    ...
    /**
     * Declared here as much as an example as an implementation: a service copied from this template
     * inherits the habit of naming its own constraints rather than answering every violation with
     * one guess.
     */
    ...
    public PageResult<User> search(UserSearchCriteria criteria, PageRequest pageRequest) {
        ...
        Page<UserJpaEntity> page = jpaRepository.findAll(UserSpecifications.matching(criteria), pageable);
    ...
    @Override
    public CursorPageResult<User> searchAfter(UserSearchCriteria criteria, CursorPageRequest pageRequest) {
        ...
        List<UserJpaEntity> rows = jpaRepository.searchAfter(
                UserSpecifications.matching(criteria), afterCreatedAt, afterId, pageRequest.size() + 1);
